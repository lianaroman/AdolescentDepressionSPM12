function firstlevel_spm12_ISSF_SRT(subj_func_dir, subj_log_dir)
% function firstlevel_spm12_ISSF_SRT(subj_func_dir)
%
% First level analysis of ISSF SRT task
% - subj_func_dir   : full path to the participant's SRT data, eg
%                     '/Volumes/cmvm/scs/groups/hwhalley-adol-imaging/MRI_scans/T001/20201009/SRT'
% - subj_log_dir    : full path to the participant's Presentation logfile
...
func_nii         = fullfile(subj_func_dir, sprintf('swu%s_SRT_brain.nii', studyID)); % The fMRI .nii preprocessed data
presentation_log = find_subfolder(subj_log_dir, '.*SelfReferentialTask'); % The Presentation task logfile
first_dir        = fullfile(subj_func_dir, 'SRT_firstlevel'); % first level analysis location
x                = unix(sprintf('mkdir %s', first_dir));
rp_file          = find_subfolder(subj_func_dir, 'rp.*_brain.txt'); % realignment parameters
data_frame       = [7:544]; % Volumes to include

% Load the regressor matfile
[a b c] = fileparts(func_nii);
[Presentation_matfile, contrasts, TR, units] = extract_ISSF_SRT(presentation_log, studyID, a);

% Where applicable, modify the Presentation_matfile nuisance column to
% include ArtRepair interpolated volumes
if(strfind(func_nii, 'swuv'))
